...
function getBlock(props, key) {
    const prop = findProp(props, key);
    if (!prop || prop.value.kind !== 'block')
        return undefined;
...
function getDuration(props, key) {
...
function compileMustProduce(props) {
...
function compilePropCondition(props, key) {
    const prop = findProp(props, key);
    if (!prop)
        return undefined;
    return compileCondition(prop.value);
}
function tryParseDuration(s) {
    try {
        return parseDuration(s);
    }
    catch {
...
function compileRollback(props) {
...
function compileOnSuccess(props) {
...
export function compile(ast) {
    ...
    const phases = ast.phases.map(compilePhase);
