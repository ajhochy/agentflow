...
export class Parser {
    ...
    expect(kind, value) {
        ...
        if (token.kind !== kind || (value !== undefined && token.value !== value)) {
            throw new ParseError(`Expected ${kind}${value ? `(${value})` : ''} but got ${token.kind}(${token.value})`, token.line, token.col);
        }
        return this.advance();
    }
    match(kind, value) {
    ...
    parse() {
        ...
        if (this.peek().kind === 'INDENT') {
            ...
            while (this.peek().kind !== 'DEDENT' && this.peek().kind !== 'EOF') {
                this.skipNewlines();
                if (this.peek().kind === 'DEDENT' || this.peek().kind === 'EOF')
                    break;
                const token = this.peek();
    ...
    parseExpression() {
        ...
        if (this.peek().kind === 'OPERATOR') {
            ...
            let condition = { kind: 'condition', left, op, right };
            // Check for 'and' / 'or' chaining
            while (this.peek().kind === 'AND' || this.peek().kind === 'OR') {
                const logic = this.advance().value;
                const nextLeft = this.parseAtom();
                if (this.peek().kind === 'OPERATOR') {
                    const nextOp = this.advance().value;
                    const nextRight = this.parseAtom();
                    const nextCondition = { kind: 'condition', left: nextLeft, op: nextOp, right: nextRight };
                    condition = { kind: 'condition', left: condition, op: logic, right: nextCondition, logic };
                }
                else {
                    condition = { kind: 'condition', left: condition, op: logic, right: nextLeft, logic };
                }
            }
            return condition;
        }
