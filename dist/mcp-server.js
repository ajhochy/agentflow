...
function loadWorkflows(dir) {
    ...
    try {
        ...
        console.error(`[agentflow] Cannot read workflows dir: ${dir} — ${err.message}`);
        return workflows;
    }
    for (const file of files) {
        ...
        try {
            ...
            console.error(`[agentflow] Skipping ${file}: ${err.message}`);
        }
    }
    return workflows;
}
...
async function main() {
    ...
    for await (const line of rl) {
        ...
        try {
            ...
            sendResponse(makeError(request.id, -32603, err.message));
        }
    }
}
function mapType(aflowType) {
    switch (aflowType) {
        case 'int':
