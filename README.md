...
npx tsx src/cli.ts check examples/code-quality.aflow

# Compile to IR JSON
npx tsx src/cli.ts compile examples/code-quality.aflow

# Validate only
npx tsx src/cli.ts validate examples/custom-domain.aflow

# Build (save IR to disk)
npx tsx src/cli.ts build examples/code-quality.aflow

# Run with MockRuntime
npx tsx src/cli.ts run examples/code-quality.aflow --input 'task="Build a REST API"'
```
